...
### Fixed
- **TypeScript Compilation**: Fixed type errors in PlansPage.tsx
  - Resolved incompatible type issues between different feature groups
  - Added proper FeatureItem type definition for consistent typing across feature arrays
  - Fixed flatMap type inference issues with explicit type assertions

### Added
- **AI Feature Flags**: Extended AI feature management with additional flags
  - Added `ai_operator_guidance_enabled` flag for AI suggestions feature
  - Added `ai_insight_persistence_enabled` flag for AI results persistence
  - Updated TenantPlanTab to display new AI features with proper Kyrgyz labels
